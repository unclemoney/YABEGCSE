...
- Manual test via Godot CLI:
  ```powershell
  & "C:\Users\danie\OneDrive\Documents\GODOT\Godot_v4.4.1-stable_win64.exe" --path "c:\Users\danie\Documents\YABEGCSE\editor" Tests/SectorDrawTest.tscn
  ```
...
- **Project Root**: `c:\Users\danie\Documents\YABEGCSE\editor`
- **Repo Root**: `c:\Users\danie\Documents\YABEGCSE`
- **Key Autoloads**: `GameSettings` (preferences), `AudioManager` (sound). Four slots reserved.
- **Key Controllers**: `EditorController` (root coordinator), `ToolSystem`, `Canvas2D`, `Viewport3D`
- **Level Data**: `LevelData` Resource class, owned by `EditorController`
- **Serializer**: `LevelSerializer` class, instantiated by `EditorController`
- **Mesh Builder**: `SectorMeshBuilder` class, reads `LevelData`, produces Godot meshes
- **Milestones**: M1 (2D canvas + sector drawing), M2 (mesh generator + walk mode), M3 (3D editing), M4 (objects), M5 (converter), M6 (environment), M7 (gameplay)
- **Debug Panel**: Press `F12` in editor. All new features need debug commands in `debug_panel.gd`.
