...
- **Level Data**: `LevelData` Resource class (`Scripts/Data/level_data.gd`), owned by `EditorController`. Drawn platforms live in `platforms: Array[PlatformData]` (`Scripts/Data/platform_data.gd`, Resource: `vertices`/`floor_height`/`ceiling_height`/`texture`/`is_visible`/`is_trigger`/`trigger_params`; `_extra` preserves unknown JSON fields) with `flagged_platforms` beside the other flag maps.
- **Serializer**: `LevelSerializer` class (`Scripts/Data/level_serializer.gd`), instantiated by `EditorController`; strings in/out, never touches the filesystem; unknown sections round-trip via `LevelData.unknown_sections`. Platforms serialize INTO the `objects` section as `{"type": "platform", "vertices": [...]}` entries — the `vertices` field distinguishes them from M4/M5 pos-based `platform` objects, which stay in `objects` untouched.
- **Mesh Builder**: `SectorMeshBuilder` (`Scripts/Runtime/sector_mesh_builder.gd`) — reads `LevelData`, produces floor/ceiling/wall meshes (one MeshInstance per distinct texture name; untextured groups keep the plain FloorMesh/CeilingMesh/WallMesh names) + `CollisionBody` (ConcavePolygonShape3D). Concave loops via `Geometry2D.triangulate_polygon`; inner loops via keyhole bridging (0.05-wide epsilon channel — Godot's ear clipping rejects coincident points, and `clip_polygons` does NOT cut rings, it returns holes as separate negative rings). **GodotPhysics concave trimeshes collide only from the backface side** — collision quads are always emitted double-wound. Sloped faces: per-vertex heights from `GeometryOps.floor_height_at`/`ceiling_height_at` (invalid slopes fall back to flat); walls/risers/lintels are per-endpoint sheared quads. UVs are world units scaled by 1/texture_size at commit (1 texel = 1 world unit); wall `offset_u`/`offset_v` shift UVs in texels. Sectors flagged with structural markers (`unclosed`, `crossing`, `overlap` — see `GeometryOps.STRUCTURAL_FLAG_MARKERS`) are skipped; cosmetic flags (slopes, floor>=ceiling) still build. Platforms (`_build_platforms`): each valid polygon emits a top face at `floor_height` + underside at `ceiling_height` (no side walls, no collision), grouped as `PlatformMesh[_tex]`; flagged platforms are skipped/counted. `build_sector_highlight(data, sector_id, y_offset)` builds the translucent floor overlay mesh for the 3D crosshair sector highlight (inner loops bridged out, slope heights honored).
...
- **3D Editing (M3)**: crosshair-driven against the walk camera (mouse stays captured). `GeometryOps.aim_from_ray` (pure math, no physics — floors/ceilings have no collision) produces the AimInfo — **floor/ceiling hits are remapped to the smallest sector containing the hit point** (2D point-in-polygon via `sector_at`), which is what makes zero-height inner sectors (flush pillars) selectable; **wall hits only count where the wall has a RENDERED face at the hit height** (`_wall_has_face_at`: solid = front floor..ceiling, portal = riser between the two floors + lintel between the two ceilings, flat heights only) — a flush inner sector's portal walls render nothing, and without this check their planes swallow the ray before the floor hit the remap needs (the live "flush pillar not selectable" bug; regression test `PlatformTest._test_angled_aim_inner_sector`, which also covers aiming through a flat doorway portal); wall hits are not remapped, void aims fall back to the raw raycast, and visible platforms are tested as horizontal planes (aim kind `&"platform"`, nearer wins). `Viewport3DView` signals `edit_input`/`edit_motion` upward; `EditorController` routes to `ToolSystem.handle_3d_input/handle_3d_motion` → `Edit3DTool`. Wheel = raise/lower face (Shift = opposite face) or wall offset_u/v; LMB drag = corner-drag slope (`floor_slope`/`ceiling_slope`: 3× [point_id, height]) — grabs the outer-loop corner nearest the crosshair hit (no distance cap; a fixed 32-unit snap was unreachable from a standing camera), and the write + undo snapshot land on the FIRST drag motion, so a bare LMB click never creates a 1-corner incomplete slope. **First motion also seeds the plane**: a lone dragged corner is an incomplete slope that validate() flags and the mesh builder renders flat, so `_seed_slope_plane` tops the face up to 3 corners (grabbed + farthest non-collinear anchors at the flat base) — one drag tilts immediately; a later drag on a corner outside the 3-entry set swaps the nearest entry out rather than growing a flagged 4th. X clears a slope; T opens the TexturePicker. **Shift+H / Shift+L** (Edit3DTool key handling, not InputMap actions) = inner sector height shortcut: match the aimed sector's ceiling/floor to its surrounding sector (`GeometryOps.surrounding_sector` = smallest-area sector whose polygon strictly contains all of the selected sector's outer vertices; -1 → status "Not an inner sector: no surrounding sector", no undo touch) via `ToolSystem.commit_match_surrounding` (one undo step; clears the face's slope so the match is visible). The aimed sector gets a barely-visible white floor tint (`Color(1, 1, 1, 0.05)`; `Viewport3DView._highlight`, mesh from `SectorMeshBuilder.build_sector_highlight`, rebuilt on aim-sector change or level rebuild). A drag is ONE undo step: snapshot at `begin_corner_drag`, live `update_corner_drag` previews without pushing undo. Look is suppressed during drags via `Viewport3DView.set_look_suppressed` (synchronous signal chain). The 3D status readout (`Edit3DTool.describe_aim`) carries the one-line slope hint. Slope pipeline debug: `GeometryOps.debug_slopes` gates `[slope]`-prefixed prints at every hop (view input → tool drag → set/remove_slope_corner → validate flags → mesh-build heights).
...
- **Environment (M6)**: schema lives in the v0 `environment` section — `{fog: {enabled, color, near, far}, ambient, sky: {mode: flat|horizon_strip, color, strip}, void: {mode: fog_color|sky_color}}`. `EnvironmentOps` (`Scripts/Data/environment_ops.gd`, static, pure) validates: `settings(data)` returns `{values, notes}` — malformed fields fall back to DEFAULTS + one note each, notes surface in the debug panel (never stored in the file). Rendering in `Viewport3DView._apply_environment`: background = void-picked color; horizon_strip adds a fog-exempt (`disable_fog = true` — exists on BaseMaterial3D in 4.4) open-ended CylinderMesh band (`SkyBand`, follows the player x/z at eye height, texture repeated 1 texel = 1 unit). Ambient goes to `AMBIENT_SOURCE_COLOR` + energy (no visual effect yet — the pipeline is unshaded). Editing: `EnvironmentPanel` (Edit menu / debug button) → `environment_applied` → `ToolSystem.commit_environment` (one undo step; undo snapshots now include `environment`). Preferences: `PreferencesPanel` → `GameSettings` persisted to `user://game_settings.cfg` via ConfigFile (tolerant load); eye height applies via `Viewport3DView.apply_preferences()` on enter_3d + apply. Debug: F11 = `debug_cycle_sky` toggles flat/horizon_strip with SKYSIDE. Fixture: `Tests/Fixtures/level_environment_v0.json` (one deliberately bad fog color).
- **GDScript/Godot gotchas (M6)**: `void` is a reserved keyword — never a variable name. `SceneTree` is not a Node — test scripts must use `root.get_node_or_null(...)`. `ClassDB.class_has_property` doesn't exist; walk `ClassDB.class_get_property_list` instead. Autoload `const`s are reachable via the instance (`settings.SETTINGS_PATH`).
- **Gameplay (M7)**: schema lives in the v0 `gameplay` section (summary in the format envelope §1, semantics in `GameplayOps`). Layers: `GameplayOps` (`Scripts/Data/gameplay_ops.gd`, static, pure; `settings(data)` → `{values, notes}` — validated view, malformed entries skipped + one note each, LevelData.gameplay never rewritten so unknown fields round-trip) → `TriggerEngine` (`Scripts/Runtime/trigger_engine.gd`, RefCounted, PURE — no nodes/LevelData/audio; events+tick in, action lists out; owns the 256-register bank, read-only 122–126, 127 = game over, near_object edges fire on (inside AND condition), run_script flattened with depth cap 8) → `GameplayRuntime` (`Scripts/Runtime/gameplay_runtime.gd`, Node owned by Viewport3DView) executes side effects: messages/warps signal up, sounds via AudioManager, `remove_object` = `Objects3D.hide_object` (runtime-only — the runtime NEVER mutates LevelData). Registers reset on every play-test entry (`enter_3d` → `start_playtest`); gameplay edits mid-play-test apply on next entry. `warp` loads the linked level (path relative to the current level file) + `restart_playtest`. Music/sounds: library-relative WAV base names via `AudioManager` (autoload, real since M7); loop needs explicit `loop_end` (default 0 = no loop). Editing: `GameplayPanel` (Edit menu / debug button) — structured rows for registers/links/music, JSON TextEdits + templates for triggers/scripts (parse-gated on Apply) — → `ToolSystem.commit_gameplay` (one undo step; snapshots include `gameplay`). The panel clamps to viewport − 20 px margins (600×360 at 640×400), scrolls its content in a `ScrollContainer` under a fixed header row with an X close button, closes on Escape, and tweens position+modulate on open/close (never scale, per the Panel Construction Standards). Debug panel: gameplay notes block, event log ring (8), register watch; F9 = `debug_gameplay_fixture`. Fixture: `Tests/Fixtures/level_gameplay_v0.json` + `ArtLibrary/MUSIC/TEST_TONE.wav`.
- **GDScript/Godot gotchas (M7)**: `var x := some_variant_returning_call()` is a compile error when the return type is `Variant` — annotate explicitly. `AudioStreamWAV` looping requires setting `loop_end` (default 0 silently means no loop). A `match` branch with multiple patterns (`"a", "b":`) is valid. JSON object keys are always Strings — `registers.initial` uses string keys in the file, int keys only in the validated view.
- **GDScript/Godot gotchas (platforms)**: `Resource.duplicate(true)` mishandles typed Array script variables — element assignment on the copy raises "Invalid assignment of index" (so `PlatformData.clone()` exists; use it for snapshots). An untyped `Array` Variant does NOT coerce into an `Array[PlatformData]` parameter — snapshot helpers take plain `Array`. `draw_dashed_line(from, to, color, width, dash)` exists on CanvasItem in 4.4 — used for the 2D platform border.
...
- **2D Tool Modes (post-M7)**: Tab / Shift+Tab cycle `Sector Draw → Vertex Edit → Wall Select → Platform Draw → Platform Edit` (`ToolSystem.cycle_tool`; the M4 object tool stays outside the cycle on the O key and Tab leaves it). The 2D/3D toggle moved from Tab to **backquote** (`toggle_mode` action). The mode label is a screen-pinned `Label` (CanvasLayer child of `Canvas2DView`, top-right with 8 px margins, growing leftward, semi-transparent black bg — the top-left belongs to the UI buttons) fed by `ToolSystem.get_tool_mode_name()`. `VertexEditTool`: 8 px (zoom-scaled via `ToolSystem._pick_scale`, fed by EditorController from `Canvas2DView.get_zoom()`) vertex pick, live drag via `begin/update/finish_vertex_drag` (one undo step); a crossing move (`GeometryOps.move_causes_crossing`) snaps back + flashes connected walls red 0.3 s + "Invalid vertex move: wall crossing" to status/debug panel. `WallSelectTool`: hover highlight, portal-only selection (boundary walls report to the status line), Delete merges sectors via `GeometryOps.check_portal_merge` (pure gate: rejects boundary walls, inner loops, multi-wall portals, self-intersecting/degenerate stitches) + `merge_sectors_at_portal` (larger-area sector survives with its heights/textures, objects untouched, one undo step via `commit_merge_portal`). Tool feedback reaches UI via `ToolSystem.status_requested` / `debug_log_requested` → EditorController → `UIPanels.log_editor_event` → DebugPanel ring (`edit:` lines). Vertex drag targets are grid-snapped; picks are not.
- **Platforms (post-M7)**: `PlatformDrawTool` (`Scripts/Editor/tools/platform_draw_tool.gd`) subclasses `DrawSectorTool` — identical loop drawing (click vertices, close on first vertex or Enter, Escape cancels), only `_try_close` differs: it calls `ToolSystem.commit_platform`, which appends a `PlatformData` to `LevelData.platforms` (floor height = sector floor at the polygon centroid, 0 in the void; ceiling = floor + 16; texture = current brush art). Platforms are overlays, NOT sectors: no walls, no containment rules, no collision. **Platform Edit mode** (`PlatformEditTool`, 5th cycle slot): LMB selects the smallest platform under the cursor (`GeometryOps.platform_at`, flagged+invisible included), Delete deletes, V toggles `PlatformData.is_visible` (serialized; invisible = skipped by `_build_platforms` and the 3D aim, dashed-outline-only in 2D), T textures via the shared `commit_texture` (aim kind `&"platform"`). The selection lives in `ToolSystem._selected_platform` so 2D and 3D share it; in 3D the Edit3DTool selects/V/Deletes aimed platforms. Validation in `GeometryOps.validate` → `flagged_platforms` (<3 points, degenerate, self-intersecting — self-intersect checked before area because a bowtie's signed area is 0); 2D canvas draws them orange-filled with a `draw_dashed_line` dotted border (red when flagged, cyan solid ring when selected), the debug panel lists them (4th arg of `set_debug_flags`/`set_flags`), and undo snapshots carry them via `EditorController._duplicate_platforms` (`PlatformData.clone()`, NOT `Resource.duplicate()` — duplicate mishandles typed Array script variables: element assignment on the copy errors).
- **Player Marker (post-M7)**: 2D canvas draws a 12 px bright-green triangle at the player position (after sector geometry, below UIPanels). Sync is signal-up/call-down: `Viewport3DView.player_moved(pos3, angle)` (emitted from `_process` on change only) and `Canvas2DView.player_moved_2d(pos2, angle)` (WASD moves the marker in 2D at GameSettings.walk_speed) both land on EditorController, which stores `player_position`/`player_angle` and calls `Canvas2DView.update_player_marker(pos2, angle)` / `Viewport3DView.update_player_position(pos2, angle)` down (the latter sets `_spawned = true` and y = floor under pos). Angle convention everywhere: radians, 0 = +X (east), positive toward +Y (map south / +Z in 3D); yaw r = atan2(-cos a, -sin a). `Canvas2DView` releases stale GUI focus on mouse press so Tab reaches the tool cycle.
- **Tests**: thirteen headless suites — `Tests/SerializerRoundTripTest.gd`, `Tests/GeometryOpsTest.gd`, `Tests/EditorShellTest.gd` (integration: drives EditorShell with synthetic input events), `Tests/MeshBuilderTest.gd`, `Tests/WalkTest.gd` (physics frames), `Tests/Edit3DTest.gd` (M3: slopes, aim raycast, 3D commits, textures), `Tests/ObjectTest.gd` (M4: object commits, views, picking, renderer smoke), `Tests/GCSImportTest.gd` (M5: real DOS 1.3 fixture pair import, mapping, round-trip, collision), `Tests/EnvironmentTest.gd` (M6: environment validation, round-trip, preferences persistence), `Tests/GameplayTest.gd` (M7: GameplayOps validation, TriggerEngine semantics, fixture chain), `Tests/VertexMergeTest.gd` (2D tool modes: vertex-move validity, drag commit path, portal merges + rejections, Tab cycle), `Tests/PlayerMarkerTest.gd` (marker sync integration, both directions + 2D WASD), `Tests/PlatformTest.gd` (platform draw/defaults/validation/mesh/serializer round-trip, inner sector height shortcut, PlatformData copy independence, zero-height inner-sector aim remap, Platform Edit tool, platform visibility). Run each with `--headless -s`; redirect stdout to a file (`> Tests/x.log`), never a pipe to `head` (deadlocks Godot's stdout). **Run a filesystem rescan in the editor after adding new `class_name` scripts** or headless runs fail with "not declared in the current scope" (stale `global_script_class_cache.cfg` — if the editor is not available, add the entry to `.godot/global_script_class_cache.cfg` by hand). **Note: `godot --headless --import` segfaults in this project** (dies in "Creating autoload scripts" — the GDAIMCPRuntime autoload); headless `-s` test runs work fine and pick up new class_names on their own. The `GDAIMCPRuntime` autoload starts a server on port 3572 in EVERY run including headless tests — a zombie test process holding 3572 takes the editor's MCP server down.
- **Debug Panel**: Press `F12` in editor (`Scripts/Editor/debug_panel.gd`). All new features need debug commands there.
- **Help / Controls**: `docs/EDITOR_CONTROLS.md` is the canonical key-binding reference — update it whenever a control changes. F1 (`help_panel` action) or the "?" button (top-right, below the mode label) opens the `HelpPanel` (`Scripts/Editor/help_panel.gd`), a clamped/scrollable viewer of that file (Escape closes).
