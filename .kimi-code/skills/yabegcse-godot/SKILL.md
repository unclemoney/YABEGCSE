...
- **Mesh Builder**: `SectorMeshBuilder` (`Scripts/Runtime/sector_mesh_builder.gd`) — reads `LevelData`, produces FloorMesh/CeilingMesh/WallMesh + `CollisionBody` (ConcavePolygonShape3D). Concave loops via `Geometry2D.triangulate_polygon`; inner loops via keyhole bridging (0.05-wide epsilon channel — Godot's ear clipping rejects coincident points, and `clip_polygons` does NOT cut rings, it returns holes as separate negative rings). **GodotPhysics concave trimeshes collide only from the backface side** — collision quads are always emitted double-wound.
...
- **Milestones**: M1 done (2D canvas, sector loop drawing, auto-split, portals, inner loops, tolerate+flag, 3-deep undo, save/load), M2 done (mesh generator, walk mode, fog toggle), M3 (3D editing), M4 (objects), M5 (converter), M6 (environment), M7 (gameplay).
